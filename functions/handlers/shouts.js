...
exports.likeShout = (req, res) => {
    ...
    shoutDocument.get()
        ...
        .then(data => {
            //If user has not liked the post, confirm the like
            if(data.empty){
...
exports.deleteShout = (req, res) => {
    ...
    document.get()
        .then((doc) => {
            //Delete the shout
            if (!doc.exists) {
                return res.status(404).json({ error: 'Shout does not exist' });
            }
